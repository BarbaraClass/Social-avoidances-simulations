...
update.network <- function(network, pn, pa, pr) {
  ...
  #potential new partners (friends or avoidances) an individual can then have (unknown to its mother): depends on Pr
  no.edges <- which(network[repro,]==0)
  rel[no.edges] <- sample(c(0,1,-1),length(no.edges),prob=c(1-2*pr,pr,pr),replace=TRUE)
